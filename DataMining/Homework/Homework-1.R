...
plot( avarageRatings$year, avarageRatings$rating, type = 'l' )


# 4.Create a boxplot for imdbRating and year, what do you see? (2 points)
boxplot( movies$imdbRating ~ movies$Year)
boxplot( movies$Year ~ movies$imdbRating )
# hishet avarage rating was during ~1970s
...
hist( movies2$tomatoUserRating*2, col = rgb( 0.5, 0.9, 0.6, 0.7 ) ) # the most frequent rating is ~3.2
hist( movies2$imdbRating, add = T, col = rgb( 0.5, 0.2, 0.6, 0.7 ) )
# from 1.5 to 4.5
...
# take only 3 needed columns that are note NA
df <- na.omit( movies2[ c('Metascore', 'imdbRating', 'Title' ) ] )
difference <- data.frame( abs( df$Metascore - 10 * df$imdbRating ) )
colnames( difference ) <- c( 'diff' )
df$Title[ which.max( difference$diff ) ]
...
# its like a cosine distance
# if sign is + and close to 1 => correlation is significant
# if cor() -> 0 => there is no correlation
# if sign is - and close to -1 => they are different ( vectors point in the opposite direction )
...
# it may depend on time too because as the time passes the value of money changes and during the 20th centyri value of 1$ is not the same as that of a during the 21 century
...
cor( movies2$actor_1_facebook_likes, movies2$cast_total_facebook_likes )    # 0.9460783
...
attrition$JobRole[ which.max( attrition$HourlyRate ) ]           # Healthcare Representative
...
# cor = 0.02428654 => there is no correlation between these factors ( it can be seen for the fraph too )
...
# PercentSalaryHike doesn't depend on Education
...
# actually there is no column churn or leaveCompany or stuff like that in the data...
# lets take the column attrition as the parameter for leaving the job
t.test( attrition$MonthlyIncome ~ attrition$Attrition )
# p-value = 4.434e-13 => they are correlated as p-value -> 0
...
# test the same thing with chisq.test
attritionGender = table( attrition$Attrition, attrition$Gender )
chisq.test( attritionGender )
# p-value = 0.2906 => not that close to 0 there isn't a significant relationship between these attributes
...
chisq.test( attrition$DistanceFromHome, attrition$DailyRate ) # p-value = 0.9608 => no correlation => independent
...
nums <- sapply( attrition, is.numeric )
cor( attrition[ ,nums ], use='complete.obs' ) # all correlations

boxplot( attrition$MonthlyIncome ~ attrition$JobLevel )             # as expected, they are highly correlated
t.test( attrition$YearsAtCompany, attrition$YearsWithCurrManager )  # p-value < 2.2e-16 => most of the workers stay with the same manager during their experience

cor( attrition$Age, attrition$TrainingTimesLastYear )     # -0.01962082 => no correlation
t.test( attrition$Age, attrition$TrainingTimesLastYear )  # p-value < 2.2e-16 which is kind of strage ( it shows that here is some correlation )
